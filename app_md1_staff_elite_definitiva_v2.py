...
def plot_team_risk_distribution(team_df):
    ...
    fig.update_layout(height=360, margin=dict(l=20,r=20,t=65,b=90), showlegend=False, title_x=0.5)
    fig.update_xaxes(tickangle=-25, automargin=True, title="")
...
def plot_team_objective_bar(team_df):
    ...
    fig.update_layout(height=400, margin=dict(l=20,r=20,t=65,b=110), showlegend=False, title_x=0.5)
    fig.update_xaxes(tickangle=-35, automargin=True, title="")
    fig.update_yaxes(automargin=True, title="objective_loss_score")
    return fig
...
def plot_team_score_trend(df):
    ...
    fig.update_layout(title="Evolución global del grupo", height=340, margin=dict(l=20,r=20,t=65,b=40), title_x=0.5, legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0))
...
def build_pdf_bytes_player_session(row, player_df):
    ...
    diag = [
        ...
        ["Loss score", f"{row['objective_loss_score']:.2f}"],
    ...
    t = Table(diag, colWidths=[4.8*cm, 10*cm])
    ...
    for fig in [radar_current_vs_baseline(row), plot_player_snapshot_compare(row), plot_objective_timeline(player_df, row["Fecha"])]:
        rl_img = fig_to_rl_image(fig, width_cm=16.5, height_cm=7.8)
...
def build_pdf_bytes_player_season(player_df, player):
    ...
    elems.append(Spacer(1, 0.2*cm))
    diag = [
        ["Riesgo actual", str(latest["risk_label"]), "Loss score", f"{latest['objective_loss_score']:.2f}"],
        ["Readiness", f"{latest['readiness_score']:.0f}", "Tendencia", str(latest["trend_label"])],
        ["Perfil", main, "Patrón", pattern],
        ["Variable dominante", f"{LABELS.get(worst_metric, 'NA')} ({'NA' if worst_value is None else f'{worst_value:.1f}%'})", "", ""],
    ]
    t = Table(diag, colWidths=[3.8*cm, 5.2*cm, 3.4*cm, 5.4*cm])
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.whitesmoke),("GRID",(0,0),(-1,-1),0.3,colors.lightgrey),("FONTNAME",(0,0),(-1,-1),"Helvetica-Bold")]))
    elems.append(t)
    elems.append(Spacer(1, 0.2*cm))
    for f in (flags or ["Sin flags adicionales"]):
    ...
    for fig in [radar_current_vs_baseline(latest), plot_objective_timeline(player_df, latest["Fecha"]), plot_metric_pct(player_df, "CMJ", latest["Fecha"]), plot_metric_pct(player_df, "RSI_mod", latest["Fecha"]), plot_metric_pct(player_df, "VMP", latest["Fecha"])]:
        rl_img = fig_to_rl_image(fig, width_cm=11.5, height_cm=5.8)
        if rl_img is not None:
            elems.append(rl_img)
...
def build_pdf_bytes_team_session(team_day, selected_date):
    ...
    for fig in [plot_team_risk_distribution(team_day), plot_team_objective_bar(team_day)]:
        rl_img = fig_to_rl_image(fig, width_cm=12.5, height_cm=6.8)
        if rl_img is not None:
            elems.append(rl_img)
            elems.append(Spacer(1, 0.15*cm))
