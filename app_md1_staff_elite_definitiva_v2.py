...
def decision_from_row(row):
    ...
    z = row.get("objective_z_score", np.nan)
    if risk == "Fatiga crítica":
        return "Intervención clara"
    if risk in ["Fatiga moderada", "Fatiga moderada-alta"]:
        return "Control estricto"
    if risk in ["Fatiga leve", "Fatiga leve-moderada"]:
        return "Control y seguimiento"
    if pd.notna(z) and z <= -1.0:
        return "Vigilar respuesta"
    return "Normal"

def state_distribution_html(player_df):
    dist = player_df["risk_label"].value_counts().reindex(RISK_ORDER, fill_value=0)
    boxes = []
    for state, n in dist.items():
        boxes.append(f"<div class='card'><div class='label'>{state}</div><div class='value'>{int(n)}</div></div>")
    return "".join(boxes)
...
def compute_metrics(df):
    ...
    for m in OBJECTIVE_METRICS:
        mean_m = df.groupby("Jugador")[m].transform("mean")
        std_m = df.groupby("Jugador")[m].transform(lambda s: s.std(ddof=0))
        df[f"{m}_cv"] = np.where(mean_m != 0, (std_m / mean_m) * 100, np.nan)

    df["objective_cv_index"] = df[[f"{m}_cv" for m in OBJECTIVE_METRICS]].mean(axis=1, skipna=True)
    df["stability_label"] = df["objective_cv_index"].apply(stability_from_cv)
    df["decision_label"] = df.apply(decision_from_row, axis=1)
    return df
...
def radar_relative_loss(row):
    ...
    for m in OBJECTIVE_METRICS:
        ...
        losses.append(0 if pd.isna(pct) else max(0, -pct))
    labels_closed = labels + [labels[0]]
    losses_closed = losses + [losses[0]]
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(
        r=losses_closed, theta=labels_closed, fill="toself",
        line=dict(color="#C62828", width=3), fillcolor="rgba(198,40,40,0.22)", name="Pérdida"
    ))
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0, 15], tickvals=[0, 2.5, 5, 10, 15], ticktext=["0", "-2.5%", "-5%", "-10%", "-15%"])),
        showlegend=False, title="Radar de pérdida relativa", height=360, margin=dict(l=10,r=10,t=35,b=10)
    )
    return fig
...
def plot_team_risk_distribution(team_df):
    ...
    fig = px.bar(temp, x="Estado", y="N", color="Estado", color_discrete_map=RISK_COLORS, title="Distribución del riesgo")
...
def plot_team_objective_bar(team_df):
    ...
    fig = px.bar(temp, x="Jugador", y="objective_loss_score", color="risk_label", color_discrete_map=RISK_COLORS, text="objective_loss_score", title="Objective loss score por jugador")
    fig.update_traces(texttemplate="%{text:.2f}", textposition="outside")
    fig.update_layout(height=320, margin=dict(l=10,r=10,t=35,b=10), showlegend=False)
    return fig

def plot_team_score_trend(df):
    temp = df.groupby("Fecha").agg(objective_loss_score=("objective_loss_score","mean"), readiness_score=("readiness_score","mean"), objective_z_score=("objective_z_score","mean")).reset_index().sort_values("Fecha")
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=temp["Fecha"], y=temp["objective_loss_score"], mode="lines+markers", name="Loss equipo"), secondary_y=False)
    fig.add_trace(go.Scatter(x=temp["Fecha"], y=temp["objective_z_score"], mode="lines+markers", name="Z-score equipo"), secondary_y=False)
    fig.add_trace(go.Scatter(x=temp["Fecha"], y=temp["readiness_score"], mode="lines+markers", name="Readiness equipo"), secondary_y=True)
    fig.update_layout(title="Evolución global del grupo", height=320, margin=dict(l=10,r=10,t=35,b=10))
    fig.update_yaxes(title_text="Loss / Z", secondary_y=False)
    fig.update_yaxes(title_text="Readiness", secondary_y=True)
    return fig
...
def coach_session_html(team_day, selected_date):
    ...
    for _, r in temp.iterrows():
        rows += f"<tr><td>{r['Jugador']}</td><td>{html_risk_badge(r['risk_label'])}</td><td>{r['CMJ_pct_vs_baseline']:.1f}%</td><td>{r['RSI_mod_pct_vs_baseline']:.1f}%</td><td>{r['VMP_pct_vs_baseline']:.1f}%</td><td>{r['objective_loss_mean_pct']:.1f}%</td><td>{r['objective_loss_score']:.2f}</td></tr>"
...
def player_session_html(row, player_df, session_df):
    ...
    current_vs_html = plotly_html(radar_current_vs_baseline(row))
    loss_radar_html = plotly_html(radar_relative_loss(row))
    snapshot_html = plotly_html(plot_player_snapshot_compare(row))
    ...
    <div class="cards">
      <div class="card"><div class="label">Decisión operativa</div><div class="value" style="font-size:20px;">{row['decision_label']}</div></div>
      <div class="card"><div class="label">Estabilidad histórica</div><div class="value" style="font-size:20px;">{row['stability_label']}</div></div>
      <div class="card"><div class="label">CV objetivo</div><div class="value">{row['objective_cv_index']:.1f}</div></div>
      <div class="card"><div class="label">Z-score objetivo</div><div class="value">{'NA' if pd.isna(row['objective_z_score']) else f'{row['objective_z_score']:.2f}'}</div></div>
    </div>
    <div class="section"><div class="title">Diagnóstico staff</div><div class="diag"><p><b>Perfil principal:</b> {main}.</p><p><b>Patrón:</b> {pattern}.</p><p><b>Variable dominante:</b> {LABELS.get(worst_metric, 'NA')} ({'NA' if worst_value is None else f'{worst_value:.1f}%'}).</p><p><b>Decisión sugerida:</b> {row['decision_label']}.</p><p>{player_comment(row)}</p></div></div>
    <div class="section"><div class="title">Flags automáticos</div><ul>{flags_html}</ul></div>
    <div class="section"><div class="title">Panel visual</div><div class="grid2">{current_vs_html}{loss_radar_html}</div><div class="grid2">{snapshot_html}{timeline_html}</div></div>
...
def player_season_html(player_df, player):
    ...
    current_vs_html = plotly_html(radar_current_vs_baseline(latest))
    loss_radar_html = plotly_html(radar_relative_loss(latest))
    timeline_html = plotly_html(plot_objective_timeline(player_df, latest["Fecha"]))
    ...
    dist_html = state_distribution_html(player_df)
    ...
    <div class="cards">
      <div class="card"><div class="label">Decisión operativa</div><div class="value" style="font-size:20px;">{latest['decision_label']}</div></div>
      <div class="card"><div class="label">Estabilidad histórica</div><div class="value" style="font-size:20px;">{latest['stability_label']}</div></div>
      <div class="card"><div class="label">CV objetivo</div><div class="value">{latest['objective_cv_index']:.1f}</div></div>
      <div class="card"><div class="label">Z-score objetivo</div><div class="value">{'NA' if pd.isna(latest['objective_z_score']) else f'{latest['objective_z_score']:.2f}'}</div></div>
    </div>
    <div class="section"><div class="title">Diagnóstico longitudinal</div><div class="diag"><p><b>Perfil principal actual:</b> {main}.</p><p><b>Patrón:</b> {pattern}.</p><p><b>Variable dominante:</b> {LABELS.get(worst_metric, 'NA')} ({'NA' if worst_value is None else f'{worst_value:.1f}%'}).</p><p><b>Decisión sugerida:</b> {latest['decision_label']}.</p><p>{player_comment(latest)}</p></div></div>
    <div class="section"><div class="title">Distribución de estados durante el periodo</div><div class="cards">{dist_html}</div></div>
    <div class="section"><div class="title">Flags actuales</div><ul>{flags_html}</ul></div>
    <div class="section"><div class="title">Panel visual</div><div class="grid2">{current_vs_html}{loss_radar_html}</div><div class="grid1">{timeline_html}</div><div class="grid2">{cmj_html}{rsi_html}</div><div class="grid1">{vmp_html}</div></div>
...
def build_pdf_bytes_player_session(row, player_df):
    ...
    diag = [["Riesgo", str(row["risk_label"])],["Loss score", f"{row['objective_loss_score']:.2f}"],["Pérdida media %", f"{row['objective_loss_mean_pct']:.1f}%"],["Readiness", f"{row['readiness_score']:.0f}"],["Decisión", row['decision_label']],["Estabilidad", row['stability_label']],["CV objetivo", f"{row['objective_cv_index']:.1f}"],["Perfil", main]]
    t = Table(diag, colWidths=[4.5*cm, 11*cm])
    ...
    elems.append(t)
    elems.append(Spacer(1, 0.2*cm))
    for fig in [radar_current_vs_baseline(row), radar_relative_loss(row), plot_objective_timeline(player_df, row['Fecha'])]:
        rl_img = fig_to_rl_image(fig, width_cm=16, height_cm=6.8)
...
def build_pdf_bytes_player_season(player_df, player):
    ...
    elems.append(Spacer(1, 0.2*cm))
    diag = [["Riesgo actual", str(latest['risk_label'])],["Loss score", f"{latest['objective_loss_score']:.2f}"],["Readiness", f"{latest['readiness_score']:.0f}"],["Decisión", latest['decision_label']],["Estabilidad", latest['stability_label']],["CV objetivo", f"{latest['objective_cv_index']:.1f}"]]
    t = Table(diag, colWidths=[4.2*cm, 6.5*cm])
    t.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),colors.whitesmoke),("GRID",(0,0),(-1,-1),0.3,colors.lightgrey),("FONTNAME",(0,0),(-1,-1),"Helvetica-Bold")]))
    elems.append(t)
    elems.append(Spacer(1, 0.2*cm))
    for fig in [radar_current_vs_baseline(latest), radar_relative_loss(latest), plot_objective_timeline(player_df, latest['Fecha'])]:
        rl_img = fig_to_rl_image(fig, width_cm=10.8, height_cm=5.6)
        if rl_img is not None:
            elems.append(rl_img)
            elems.append(Spacer(1, 0.1*cm))
    summary = [["Fecha","Loss","Pérdida %","Readiness","Riesgo","Tendencia"]]
    ...
    for _, r in temp.iterrows():
        summary.append([str(r['Fecha']), f"{r['objective_loss_score']:.2f}", f"{r['objective_loss_mean_pct']:.1f}%", f"{r['readiness_score']:.0f}", str(r['risk_label']), str(r['trend_label'])])
    t2 = Table(summary, repeatRows=1)
...
def build_pdf_bytes_team_session(team_day, selected_date):
    ...
    for fig in [plot_team_risk_distribution(team_day), plot_team_objective_bar(team_day)]:
        rl_img = fig_to_rl_image(fig, width_cm=11.5, height_cm=6.0)
        if rl_img is not None:
            elems.append(rl_img)
            elems.append(Spacer(1, 0.1*cm))
...
def page_jugador(metrics_df):
    ...
    with c7: kpi("Decisión", row["decision_label"], "operativa")
    with c8: kpi("Estabilidad", row["stability_label"], f"CV {row['objective_cv_index']:.1f}")

    a,b = st.columns(2)
    with a: st.plotly_chart(radar_current_vs_baseline(row), use_container_width=True)
    with b: st.plotly_chart(radar_relative_loss(row), use_container_width=True)
    c,d = st.columns(2)
...
def page_admin(base_df):
    ...
    st.download_button("Descargar base CSV", data=base_df.to_csv(index=False).encode("utf-8"), file_name="md1_staff_elite_definitiva_v4.csv", mime="text/csv")
